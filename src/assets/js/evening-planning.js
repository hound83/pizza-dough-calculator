...
function missingTemplateChoices(t){
  return {pizzas:t.pizzas.filter(p=>!pizzaRecipes.some(r=>r.id===p.recipeId)),sauces:[...new Set([t.sauce.type,...t.pizzas.map(p=>p.custom.sauceOverride).filter(Boolean)])].filter(id=>!sauces[id])};
}
function templateResolutionHtml(){
  const t=pendingEveningTemplate;if(!t)return '';
  const missing=missingTemplateChoices(t);
  return `<div class="restore-preview" id="templateResolution"><h3>${L('Kies vervangende recepten','Choose replacement recipes')}</h3><p>${L('Deze recepten ontbreken in de huidige catalogus. Kies bewust een vervanging; het bewaarde sjabloon blijft intact.','These recipes are missing from the current catalogue. Choose replacements explicitly; the saved template stays intact.')}</p>${missing.pizzas.map(p=>`<label>${esc(currentLang==='en'?p.snapshot.nameEn:p.snapshot.name)}<select class="field" data-resolve-pizza="${esc(p.id)}"><option value="">${L('Kies een recept','Choose a recipe')}</option>${pizzaRecipes.map(r=>`<option value="${esc(r.id)}">${esc(currentLang==='en'?r.nameEn||r.name:r.name)}</option>`).join('')}</select></label>`).join('')}${missing.sauces.map(id=>`<label>${L('Ontbrekende saus','Missing sauce')}: ${esc(id)}<select class="field" data-resolve-sauce="${esc(id)}"><option value="">${L('Kies een saus','Choose a sauce')}</option>${Object.entries(sauces).map(([id,s])=>`<option value="${esc(id)}">${esc(currentLang==='en'?s.nameEn||s.name:s.name)}</option>`).join('')}</select></label>`).join('')}<div class="btnrow">${eveningButton('resolve-template',L('Met deze keuzes gebruiken','Use these choices'))}${eveningButton('cancel-template',L('Annuleren','Cancel'),'','ghost')}</div></div>`;
}
function eveningArchiveHtml(e){
  return `<details class="evening-archive"><summary>${L('Momenten en metingen bekijken','View checkpoints and readings')}</summary><p>${L('Geplande bakstart','Planned bake start')}: ${niceDate(new Date(e.bakeAt))}</p>${e.runs.map((r,i)=>r.batch?`<h4>${L('Beurt','Run')} ${i+1} · ${r.balls} ${L('bollen','balls')}</h4>${WorkflowCore.eventKeys(r.batch.route).map(key=>`<div class="checkpoint-row"><span>${batchEventName(key)}<small>${r.batch.events[key]!=null?niceDate(new Date(r.batch.events[key])):WorkflowCore.eventDone(r.batch,key)?L('Gedaan, tijd onbekend','Done, time unknown'):L('Niet geregistreerd','Not recorded')}</small></span></div>`).join('')}<p>${L('Gemeten na kneden','Measured after kneading')}: ${r.batch.measurements.doughTemp==null?'—':fmt(r.batch.measurements.doughTemp,1)+' °C'} · ${L('Gemeten koelkast','Measured fridge')}: ${r.batch.measurements.fridgeTemp==null?'—':fmt(r.batch.measurements.fridgeTemp,1)+' °C'}</p>`:`<p>${L('Beurt','Run')} ${i+1}: ${L('Niet gestart','Not started')}</p>`).join('')}</details>`;
}
function renderEveningCollection(){
  const templates=evenings.templates,history=evenings.history;
  const sharePreview=eveningShare?`<div class="restore-preview" id="eveningSharePreview"><h3>${L('Dit ga je delen','What you will share')}</h3><p>${L('Deegrecept, sauzen, toppings, volgorde, verdeling en materiaal. Namen, programmanotities, datums, metingen en werkelijke geschiedenis zijn weggelaten.','Dough recipe, sauces, toppings, order, allocation and equipment. Names, programme notes, dates, readings and actual history are omitted.')}</p><ol>${eveningShare.template.pizzas.map(p=>`<li>${esc(currentLang==='en'?p.snapshot.nameEn:p.snapshot.name)}</li>`).join('')}</ol><details><summary>${L('Volledige bestandsinhoud','Full file contents')}</summary><pre class="share-file-content">${esc(JSON.stringify(eveningShare,null,2))}</pre></details><div class="btnrow">${eveningButton('download-shared',L('Bestand downloaden','Download file'))}${eveningButton('cancel-share',L('Annuleren','Cancel'),'','ghost')}</div></div>`:'';
...
function applyEveningTemplate(template){
  ...
  const missing=missingTemplateChoices(t);
  if(missing.pizzas.length||missing.sauces.length){pendingEveningTemplate=t;renderEveningCollection();$('eveningCollectionDetails').open=true;return;}
  pendingEveningTemplate=null;
...
async function handleEveningAction(button){
  ...
  if(temperature&&activeBatch()){
    ...
    if(value!==liveMeasurementValue('doughTemp'))setLiveMeasurement('doughTemp',raw);
  }
  ...
  if(action==='start')startEvening();
    ...
    eveningShare=EveningCore.exportTemplate(captureEveningTemplate());renderEveningCollection();$('eveningCollectionDetails').open=true;
  }else if(action==='download-shared'){downloadEveningFile(eveningShare,'pizza-evening.json');}
  else if(action==='cancel-share'){eveningShare=null;renderEveningCollection();}
  ...
  else if(action==='cancel-template'){pendingEveningTemplate=null;renderEveningCollection();}
  else if(action==='resolve-template'){
    const t=WorkflowCore.clone(pendingEveningTemplate);if(!t)return;
    for(const select of document.querySelectorAll('[data-resolve-pizza]')){
      if(!pizzaRecipes.some(r=>r.id===select.value))throw new Error(L('Kies voor elk ontbrekend recept een vervanging.','Choose a replacement for each missing recipe.'));
      const p=t.pizzas.find(p=>p.id===select.dataset.resolvePizza);p.recipeId=select.value;p.custom={recipeId:select.value,excluded:{},noSauce:false,extraCheese:false,pizzaStyle:'traditional',sauceOverride:null};
    }
    for(const select of document.querySelectorAll('[data-resolve-sauce]')){
      if(!sauces[select.value])throw new Error(L('Kies voor elke ontbrekende saus een vervanging.','Choose a replacement for each missing sauce.'));
      if(t.sauce.type===select.dataset.resolveSauce)t.sauce.type=select.value;
      t.pizzas.forEach(p=>{if(p.custom.sauceOverride===select.dataset.resolveSauce)p.custom.sauceOverride=select.value;});
    }
    applyEveningTemplate(t);
  }
