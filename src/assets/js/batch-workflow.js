...
function restoreRecipe(snapshot){
  const r=WorkflowCore.recipe(snapshot);if(!r)return false;
  suppressCustom=true;
  for(const [key,value] of Object.entries(r.fields))$(key).value=value;
...
function closeBatch(){
  ...
  $('bakeDay').value='';workshopNotice=L('Batch bewaard. Je kunt een nieuw plan maken.','Batch saved. You can make a new plan.');
...
function resumeBatch(id){
  ...
  restoreRecipe(b.recipe);liveMeasurements={doughTemp:null,fridgeTemp:null};
...
function renderBatchPanels(){
  const b=activeBatch(),notice=workshopNotice?`<p class="info" role="status">${esc(workshopNotice)}</p>`:'';
  if(!b){
...
function replaceWorkshopPanel(id,html){
  ...
  root.innerHTML=html;root._workshopHtml=html;
...
function wireWorkshopEvents(){
  for(const id of workshopPanels){
    ...
    root.addEventListener('click',event=>{
      ...
      collectWorkshopDrafts();workshopNotice='';
      try{
    ...
    root.addEventListener('change',event=>{
      const el=event.target;if(el.dataset.draft!==undefined)workshopDrafts[el.id]=el.value;
