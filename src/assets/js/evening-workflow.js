...
function eveningFailure(error){
  const messages={
    ...
    'missing-previous':L(`Registreer eerst “${batchEventName(kitchenCurrentKey())}”; de tijd mag onbekend zijn.`,`First record “${batchEventName(kitchenCurrentKey())}”; its time may be unknown.`),
...
function renderKitchen(c){
  if(holdPanelRender('kitchenWorkspace'))return;
  const e=activeEvening(),b=activeBatch(),run=selectedRun();
  if(!e){eveningPanel('kitchenWorkspace',`<div class="kitchen-heading"><div class="kitchen-heading-copy"><span class="kitchen-hero-icon" aria-hidden="true">🥣</span><div><span class="eyebrow">${L('Van ingrediënten naar pizza','From ingredients to pizza')}</span><h2>${L('Jouw keuken','Your kitchen')}</h2><p>${L('Alle stappen staan hieronder klaar. Start de registratie zodra je gaat mengen.','Your steps are ready below. Start tracking when you begin mixing.')}</p><div class="kitchen-meta"><span>${pizzaCountLabel(c.pizzas)} · ${fmt(c.targetDiameter,1)} cm</span><span>${mixerMethodName(currentMethod)}</span></div></div></div>${eveningButton('start',L('Start deeg','Start dough'),'','primary')}</div>${eveningNotice?`<p class="info" role="status">${esc(eveningNotice)}</p>`:''}`);return;}
  const key=kitchenCurrentKey(b),view=WorkflowCore.timeline(b);
  ...
  const moments=WorkflowCore.eventKeys(b.route).map(k=>`<div class="checkpoint-row"><span>${batchEventName(k)}<small>${b.events[k]!=null?niceDate(new Date(b.events[k])):WorkflowCore.eventDone(b,k)?L('Gedaan, tijd onbekend','Done, time unknown'):L('Nog niet geregistreerd','Not recorded yet')}</small></span>${k==='start'?'':`<div>${eveningButton('correct-event',L('Tijd aanpassen','Correct time'),`data-key="${k}"`)}${k===key?eveningButton('unknown-event',L('Gedaan, tijd onbekend','Done, time unknown'),`data-key="${k}"`):''}</div>`}</div>`).join('');
  ...
  eveningPanel('kitchenWorkspace',`<div class="kitchen-heading"><div class="kitchen-heading-copy"><span class="kitchen-hero-icon" aria-hidden="true">🥣</span><div><span class="eyebrow">${L('Keuken','Kitchen')}${e.runs.length>1?` · ${L('Beurt','Run')} ${selected+1}/${e.runs.length}`:''}</span><h2>${title}</h2><p>${pizzaCountLabel(c.pizzas)} · ${L('Geplande eerste pizza in de oven','Planned first pizza in the oven')}: <b>${niceDate(new Date(e.bakeAt))}</b></p>${zone}</div></div>${eveningButton('wake',eveningWakeLock&&!eveningWakeLock.released?L('Scherm blijft aan','Screen stays awake'):L('Scherm aanhouden','Keep screen awake'),'aria-pressed="'+String(!!eveningWakeLock&&!eveningWakeLock.released)+'"','ghost')}</div>${runNav}${eveningNotice?`<p class="info" role="status">${esc(eveningNotice)}</p>`:''}${key!=='bulkStart'?`<p>${cues}</p>`:''}${key&&key!=='bake'?`<div class="kitchen-current-action"><div><span class="eyebrow">${L('Werkelijk moment vastleggen','Record actual checkpoint')}</span><p>${L('Verwacht','Expected')}: <b>${nextTime}</b></p></div>${eveningButton('event-now',batchEventName(key),`data-key="${key}"`,'primary')}</div>`:''}<div class="btnrow">${e.undo?eveningButton('undo',L('Laatste handeling ongedaan maken','Undo latest action'),'','ghost'):''}${eveningButton('help',L('Deeg reageert anders?','Dough behaving differently?'),'','ghost')}</div><div class="kitchen-tools"><details id="kitchenHistory"><summary>${L('Momenten aanvullen of corrigeren','Complete or correct checkpoints')}</summary>${moments}${correction}${b.route==='hybrid'?`<p class="hint">${L('Uit de koelkast is geen geregistreerd opbolmoment. Opbollen blijft een aparte handeling binnen de bestaande eindrijstijd.','Fridge-out is not a recorded shaping time. Shaping remains a separate action within the existing final-proof allowance.')}</p>`:''}</details><details id="kitchenTimers"><summary>${L('Rusttimer en herinneringen','Rest timer and reminders')}</summary>${eveningField('restTimerMinutes',L('Timer (minuten)','Timer (minutes)'),workshopDrafts.restTimerMinutes||5,'number','min="1" max="1440"')}<div class="btnrow">${eveningButton('start-timer',L('Timer starten','Start timer'))}${eveningButton('calendar',L('Planning naar agenda (.ics)','Planning to calendar (.ics)'))}</div><p class="hint">${L('Een timer verandert je planning niet. Bij extra rust kun je de resterende tijden apart aanpassen. De pagina garandeert geen alarm als zij gesloten of gepauzeerd is. Een agenda-export is een momentopname.','A timer does not change the plan. For extra rest, adjust remaining times separately. The page cannot guarantee an alarm when closed or suspended. A calendar export is a snapshot.')}</p></details></div>${e.timer?`<p class="timer-status" id="eveningTimer" role="status"></p>${eveningButton('stop-timer',L('Timer stoppen','Stop timer'),'','ghost')}`:''}<details id="bakingWorkspace" ${key==='bake'||!key||e.pizzas.some(p=>p.events.in!=null)?'open':''}><summary>${L('Bakvolgorde','Baking queue')} · ${e.pizzas.length} ${L('pizza’s','pizzas')}</summary><div id="bakingQueue">${bakingQueueHtml(c,e)}</div></details><div class="btnrow">${eveningButton('finish',L('Avond bewaren en afsluiten','Save and finish evening'),'','ghost')}</div>`);
  const bakingNow=key==='bake'||!key||e.pizzas.some(p=>p.events.in!=null);
  if(bakingNow){
    const panel=$('kitchenWorkspace'),queue=$('bakingWorkspace');
    const anchor=panel.querySelector('.run-switch')||panel.querySelector('.kitchen-heading');
    if(anchor&&queue&&queue.previousElementSibling!==anchor)anchor.after(queue);
    if(kitchenQueueOpenedFor!==e.id)queue.open=true;
    kitchenQueueOpenedFor=e.id;
  }else kitchenQueueOpenedFor='';
  renderEveningTimer();
}
...
function bakingQueueHtml(c,e){
  ...
  const nextRun=next?e.runs.find(r=>r.id===next.runId):null,nextBake=nextRun?.batch?WorkflowCore.timeline(nextRun.batch).times.bake:null;
  const nextTiming=nextRun?.batch?(nextBake==null?L('Prognose onbekend door ontbrekende werkelijke tijden.','Forecast unknown because actual times are missing.'):`${L('Verwachte bakstart voor deze beurt','Expected bake start for this run')}: ${niceDate(new Date(nextBake))}.`):L('Deze mixerbeurt is nog niet gestart.','This mixer run has not started.');
