...
The Worker accepts Bug, Idea, Calculation/recipe, and Translation feedback. It has no email field, rejects email addresses in public text, neutralizes accidental GitHub mentions, caps request and field sizes, and silently drops honeypot submissions. After successful Turnstile validation, a native Cloudflare binding rate-limits issue attempts before GitHub; invalid tokens cannot consume the shared issue quota, and its key is the allowed application origin rather than a shared mobile IP. Diagnostics are off by default and allowlist only app version, language, display mode, output mode, wizard page, and viewport. `feedback-worker/src/index.js` discards every other diagnostic key.
