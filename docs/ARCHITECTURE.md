...
`tests/test_v50.js` runs all 70 functional regressions against both the modular source and the standalone bundle. Mixer-specific coverage protects the bilingual staged KitchenAid and Kenwood guidance, the separate 30-minute cold-autolyse and 20-minute hydration-rest paths, preparation-time planning, temperature/windowpane stop criteria, and optional manual finishing without changing recipe calculations. Basic-mode coverage distinguishes ordinary recipe inputs from intentional technical overrides.
