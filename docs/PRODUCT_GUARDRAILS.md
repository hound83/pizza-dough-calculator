...
These hashes cover the v1.3.0 candidate that adds optional anonymous feedback on top of the v1.2.0 calculation candidate. They are pre-production evidence rather than a release tag; the Worker endpoint and Turnstile site key remain intentionally blank until deployment and the real end-to-end issue check.
