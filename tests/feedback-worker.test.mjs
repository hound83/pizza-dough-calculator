...
test('valid anonymous feedback verifies Turnstile and creates a bounded GitHub issue',async()=>{
  ...
  const payload=validPayload({
    ...
    diagnostics:{
      appVersion:'1.2.0',language:'en',experienceMode:'basic',outputMode:'full',wizardPage:3,viewport:'320x568',
  ...
  assert.match(issue.body,/"appVersion": "1\.2\.0"/);
...
test('diagnostic and issue-body helpers only retain approved non-recipe context',()=>{
  const diagnostics=sanitizeDiagnostics({
    appVersion:'1.2.0',language:'nl',experienceMode:'full',outputMode:'dough',wizardPage:1,viewport:'1280x900',
    recipe:{hydration:63},bakeLog:['private'],email:'person@example.com'
  });
  assert.deepEqual(diagnostics,{appVersion:'1.2.0',language:'nl',experienceMode:'full',outputMode:'dough',wizardPage:1,viewport:'1280x900'});
